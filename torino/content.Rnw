...
\begin{center}
<<eval-ux-ui, fig=TRUE, echo=FALSE, height=2.5, width=4>>=
ps.options(pointsize=8,family="serif")
bonoloui <- read.table("../bonoloui.dat", header=T, sep="\t")
par(xpd=T, mar=par()$mar+c(0,0.5,0,5), cex=0.7)
barplot(as.matrix(bonoloui), beside=TRUE, family="serif", xlab="Average UX ratings", ylab="Survey Participants", ylim=c(0,5), col=rainbow(7), cex.axis=0.7, las=1, space=c(0, 3), names.arg=c("Search&\nBrowse", "Navigability", "Similarity", "General"), cex=0.7)
op <- par(family = "serif")
legend(45,4.5, c("Intuitive", "Simple", "Satisfied", "Expected", "Response", "Effective", "Beneficial"), cex=0.7, fill=rainbow(7));
box("inner", "solid")
@  
\end{center}
...
\begin{center}
<<>>=
<<eval-perf, fig=TRUE, echo=FALSE, height=2.5, width=4>>=
...
plot(bonoloperf$Initial, type="o", col=plot_colors[1], ylim=c(0,10), axes=FALSE, ann=FALSE, cex=0.7)
op <- par(family = "serif")
axis(1, at=1:6, lab=c("1,024", "2,048", "4,096", "8,192", "16,384", "32,768"), cex=0.7)
axis(2, las=1, at=2*0:5, cex=0.7)
lines(bonoloperf$Page, type="o", pch=22, lty=2, col=plot_colors[2], cex=0.7)
title(xlab="Files in directory", cex=0.7)
title(ylab="Time (seconds)", cex=0.7)
